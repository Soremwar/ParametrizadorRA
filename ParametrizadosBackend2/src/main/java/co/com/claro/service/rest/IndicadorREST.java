...
public class IndicadorREST{
    ...
    public List<IndicadorDTO> find(
        ...
        List<IndicadorDTO> lstDTO = null; //lst.stream().map(item -> item.toDTO()).distinct().sorted(comparing(IndicadorDTO::getId)).collect(toList());
    ...
    public List<IndicadorDTO> findByAnyColumn(@QueryParam("texto") String texto){
        ...
        List<IndicadorDTO> lstDTO = null; //lst.stream().map(item -> item.toDTO()).sorted(comparing(IndicadorDTO::getId)).collect(toList());
        List<IndicadorDTO> lstFinal = (List<IndicadorDTO>)(List<?>) lstDTO;
        return lstFinal;        
    }
