...
public class EscenarioREST {
    ...
    public List<EscenarioDTO> find(
        ...
        List<PadreDTO> lstDTO = null; //lst.stream().map(item -> item.toDTO()).distinct().sorted(comparing(EscenarioDTO::getId)).collect(toList());
    ...
    public List<EscenarioDTO> findByAnyColumn(@QueryParam("texto") String texto){
        ...
        List<PadreDTO> lstDTO = null; //lst.stream().map(item -> item.toDTO()).sorted(comparing(EscenarioDTO::getId)).collect(toList());
        List<EscenarioDTO> lstFinal = (List<EscenarioDTO>)(List<?>) lstDTO;
        return lstFinal;        
    }
